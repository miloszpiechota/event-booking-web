export const reverseGeocode = async (
  ...
  try {
    ...
    if (data && data.address) {
      ...
      const { road, house_number, postcode, city, town, village, country } = data.address;
      const e_street = road || "";
      const e_apartment_number = house_number || "";
      const e_zip_code = postcode || "";
      const e_city = city || town || village || "";
      const e_country = country || "";
      return { e_street, e_apartment_number, e_zip_code, e_city, e_country };
    } else {
      return false;
    }
  } catch (error) {
